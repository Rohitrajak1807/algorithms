//A very special case of the naive approach. This case assumes that each character in the pattern is distinct,
//under this assumption, this approach takes O(n) time

#include <iostream>
#include <string_view>

void distinct_naive_string_matcher(const std::string &str, const std::string &pattern);
...
int main() {
    ...
    const std::string pattern = "video";
    distinct_naive_string_matcher(str, pattern);
    return 0;
}
...
void distinct_naive_string_matcher(const std::string &str, const std::string &pattern) {
    ...
    for (size_t i = 0; i <= str_len - pattern_len; ++i) {
        ...
        std::string_view slice = std::string_view(str.begin() + i, str.begin() + i + pattern_len);
        for(size_t j = 0; j < pattern_len; j++) {
            if(slice[j] == pattern[j]) {
                longest_match++;
            }
            if(longest_match == pattern_len) {
